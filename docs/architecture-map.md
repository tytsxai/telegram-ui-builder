...
| `src/lib/dataAccess.ts` | Single gateway for Supabase CRUD with retry/backoff and structured logging; shapes payloads via `TablesInsert/Update`; normalizes writes to the active owner and strips `user_id` from updates. | Supabase client, `supabaseRetry`, generated Supabase types. Encodes table/column names and share token semantics. |
...
- Offline queue replay couples `pendingQueue` with `SupabaseDataAccess` behavior and `user.id` scoping; in-flight edits rely on container-managed `lastSavedSnapshot`, not queue state. Queued saves can start with local temporary ids, so replay must remap final Supabase ids through screen links, navigation, and entry state.
